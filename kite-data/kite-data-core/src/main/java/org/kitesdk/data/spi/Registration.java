...
public class Registration {
  ...
  private static final Map<URIPattern, OptionBuilder<DatasetRepository>>
      REPO_URIS = Maps.newLinkedHashMap();

  private static final Map<URIPattern, OptionBuilder<DatasetRepository>>
      DATASET_URIS = Maps.newLinkedHashMap();
  ...
  public static void register(URIPattern repoPattern, URIPattern datasetPattern,
    ...
    REPO_URIS.put(repoPattern, repoBuilder);
    DATASET_URIS.put(datasetPattern, repoBuilder);
  }

  public static Pair<DatasetRepository, Map<String, String>>
      lookupRepoUri(URI repoUri) {
    for (URIPattern pattern : REPO_URIS.keySet()) {
      Map<String, String> match = pattern.getMatch(repoUri);
      if (match != null) {
        return Pair.of(REPO_URIS.get(pattern).getFromOptions(match), match);
      }
    }
  ...
  public static Pair<DatasetRepository, Map<String, String>>
    ...
    for (URIPattern pattern : DATASET_URIS.keySet()) {
      Map<String, String> match = pattern.getMatch(datasetUri);
      if (match != null) {
        return Pair.of(DATASET_URIS.get(pattern).getFromOptions(match), match);
      }
    }
  ...
  static {
    ...
    LOG.debug("Registered repository URIs:\n\t" +
        Joiner.on("\n\t").join(REPO_URIS.keySet()));
    LOG.debug("Registered dataset URIs:\n\t" +
        Joiner.on("\n\t").join(DATASET_URIS.keySet()));
  }
}
