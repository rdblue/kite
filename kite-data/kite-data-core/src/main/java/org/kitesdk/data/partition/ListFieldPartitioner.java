...
public class ListFieldPartitioner<S> extends FieldPartitioner<S, Integer> {
  ...
  private static <S> int cardinality(List<Set<S>> values) {
    // TODO: should be: return values.size();
    // cardinality is "the number of buckets in the partition", which refers to
    // the function image, not the domain, so this impl is wrong
    int c = 0;
